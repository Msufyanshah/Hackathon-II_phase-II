# Data Model: Phase II Overview - Spec-Driven Todo Application
...
This document defines the data models for the Phase II Todo application. All models follow constitutional requirements using SQLModel as the single authoritative ORM layer with UUID primary keys for security and scalability. The models serve as the foundation for the backend's domain model layer and are consumed by persistence logic and API handlers.
...
### AuthToken Model (Session Management)
- **Name**: AuthToken
- **Primary Key**: id (UUID4) - Unique identifier for each token record
- **Fields**:
  - id: UUID4 (Primary Key, default: gen_random_uuid())
  - user_id: UUID4 (Foreign Key to User.id, required)
...
- **Validation Rules**:
  ...
  - token_hash must be securely hashed
