    ...
    <div className="min-h-screen flex bg-bg-primary overflow-hidden">
      <div className="hidden lg:flex lg:w-1/2 relative items-center justify-center p-12">
        ...
        <div className="relative z-10 max-w-md">
          <div className="flex items-center gap-3 mb-8">
            <div className="w-10 h-10 rounded-lg border-2 border-accent-cyan flex items-center justify-center"><Check className="w-5 h-5 text-accent-cyan" /></div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPassword ? 'text' : 'password'} placeholder="Min. 8 characters" value={password} onChange={(e) => setPassword(e.target.value)} className="glass-input pl-10 pr-10" required />
            ...
            {password && (
              <div className="space-y-2">
                <div className="flex gap-1">{[1, 2, 3, 4].map((i) => <div key={i} className="h-1 flex-1 rounded-full transition-colors" style={{ background: i <= strength.score ? strength.color : 'var(--glass-border)' }} />)}</div>
                <p className="text-xs" style={{ color: strength.color }}>{strength.label}</p>
              </div>
            )}
            <GradientButton fullWidth disabled={isLoading} type="submit">Create Account</GradientButton>
          ...
          <p className="mt-8 text-center text-sm text-text-secondary">Already have an account? <Link href="/login" className="text-accent-violet hover:underline font-medium">Sign in</Link></p>
        </motion.div>
      </div>
    </div>
  )
