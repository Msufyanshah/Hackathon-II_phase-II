  ...
  return (
    <div className="min-h-screen bg-bg-primary">
      ...
      <div className="ml-[260px] min-h-screen">
        <TopBar title="Dashboard" subtitle={`Welcome back, ${user?.username || 'User'}! Here's your overview.`} searchPlaceholder="Search tasks..." />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <GlassCard className="p-5">
                <div className="flex items-center gap-2 mb-4 text-text-secondary"><Plus className="w-4 h-4" /><span className="text-sm font-medium">Quick Add Task</span></div>
                <div className="space-y-3">
                  <GlassInput placeholder="What needs to be done?" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && newTitle.trim() && (createTask(newTitle, ''), setNewTitle(''))} />
                  <div className="flex gap-3">
                    <GradientButton onClick={() => { if (newTitle.trim()) { createTask(newTitle, ''); setNewTitle(''); } }}>Add Task</GradientButton>
                    <button className="btn-secondary text-sm" onClick={() => setShowModal(true)}>+ Description</button>
                  </div>
                </div>
              </GlassCard>
              {/* Recent Tasks */}
              <GlassCard className="p-5">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-sm font-medium text-text-secondary flex items-center gap-2"><Zap className="w-4 h-4 text-accent-violet" />Recent Tasks</h2>
                  <a href="/tasks" className="text-xs text-accent-violet hover:underline">View All →</a>
                </div>
                {loading ? <div className="space-y-3">{[1, 2, 3].map((i) => <div key={i} className="h-12 rounded-xl bg-bg-tertiary/50" />)}</div> :
                  recentTasks.length === 0 ? <p className="text-center py-8 text-text-muted text-sm">No tasks yet — create your first one!</p> :
                  <div className="space-y-2">
                    {recentTasks.map((task) => (
                      <motion.div key={task.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-3 p-3 rounded-xl hover:bg-glass-bg transition-colors group">
                        <AnimatedCheckbox checked={task.completed} onChange={() => toggleTask(task)} />
                        <div className="flex-1 min-w-0">
                          <p className={`text-sm font-medium truncate ${task.completed ? 'line-through text-text-muted' : 'text-text-primary'}`}>{task.title}</p>
            ...
            <div className="space-y-6">
              <GlassCard className="p-5 flex flex-col items-center justify-center">
                <div className="flex items-center gap-2 mb-4 text-text-secondary"><Zap className="w-4 h-4 text-accent-violet" /><span className="text-sm font-medium">Progress</span></div>
