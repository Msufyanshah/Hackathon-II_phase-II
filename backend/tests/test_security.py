...
class TestCreateAccessToken:
    ...
    def test_create_token_with_expiration(self, test_user):
        ...
        token = create_access_token(data={"sub": str(test_user.id)}, expires_delta=expires_delta)
        decoded = jwt.decode(token, settings.SECRET_KEY, algorithms=[settings.ALGORITHM])
...
class TestVerifyToken:
    ...
    def test_verify_expired_token(self, test_user):
        ...
        token = create_access_token(data={"sub": str(test_user.id)}, expires_delta=expired_delta)
        payload = verify_token(token)
        assert payload is None
